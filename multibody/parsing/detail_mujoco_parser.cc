  ...
  void ParseEquality(XMLElement* node) {
    ...
    for (XMLElement* connect_node = node->FirstChildElement("connect");
      ...
      if (ParseStringAttribute(connect_node, "body1", &body1)) {
        ...
        Vector3d p_AP;
        if (!ParseVectorAttribute(node, "anchor", &p_AP)) {
          Error(*node,
        ...
        if (!ParseStringAttribute(connect_node, "site1", &site1) ||
          ...
          Warning(*node,
                  "connect node uses site1 and site2, which are not supported "
                  "yet. This constraint will be ignored.");
        }
      }

      WarnUnsupportedAttribute(*node, "active");
      WarnUnsupportedAttribute(*node, "solref");
      WarnUnsupportedAttribute(*node, "solimp");
    }

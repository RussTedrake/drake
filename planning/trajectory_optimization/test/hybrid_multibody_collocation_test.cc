...
GTEST_TEST(HybridMultibodyCollocationTest, BallOnTheFloor) {
  ...
  HybridMultibodyCollocation hybrid(*diagram, *context, kTimeStep, kTimeStep);
...
GTEST_TEST(HybridMultibodyCollocationTest, BallDrop) {
  ...
  auto* ground_phase = hybrid.AddModeWithInelasticImpact(
      "ground", kNumTimeSteps, hybrid.GetContactPairCandidates());
...
// A ball (sphere) drops onto an inclined plane, and rolls down into a corner
// (with another inclined plane). This tests the impact constraints when the
// multiple contacts are involved during the impact.
//
// Note: There is no hinge (no rolling dof) on the ball, because otherwise the
// ball will try to roll up the second ramp on impact; this would not test the
// multiple active contacts concept.
GTEST_TEST(HybridMultibodyCollocationTest, DropAndRollIntoCorner) {
  const std::string xml = R"""(
<mujoco model="test">
<worldbody>
    <geom name="ramp1" type="box" size="50 50 50" pos="0 0 -57.735"
        euler="0 30 0"/>
    <geom name="ramp2" type="box" size="50 50 50" pos="0 0 -57.735"
        euler="0 -30 0"/>
    <body name="body">
    <joint type="slide" name="x" axis="1 0 0" />
    <joint type="slide" name="z" axis="0 0 1" />
    <geom name="ball" type="sphere" size="1" mass="1" rgba="0.6 0.2 0.2 1"/>
    </body>
</worldbody>
</mujoco>)""";
  ...
  const double kMaxTimeStep = 1.0;
  HybridMultibodyCollocation hybrid(*diagram, *context, kMinTimeStep,
                                    kMaxTimeStep);
  auto& prog = hybrid.prog();
  auto& plant = diagram->plant();
  auto& inspector = hybrid.model_inspector();
  GeometryId ball = inspector.GetGeometryIdByName(
      plant.GetBodyFrameIdOrThrow(plant.GetBodyByName("body").index()),
      geometry::Role::kProximity, "test::ball");
  GeometryId ramp1 = inspector.GetGeometryIdByName(
      inspector.world_frame_id(), geometry::Role::kProximity, "ramp1");
  GeometryId ramp2 = inspector.GetGeometryIdByName(
      inspector.world_frame_id(), geometry::Role::kProximity, "ramp2");
  ...
  auto* ramp1_phase = hybrid.AddModeWithInelasticImpact(
      "ramp1", kNumTimeSteps,
      {HybridMultibodyCollocation::ContactPair(ball, ramp1)});
  ramp1_phase->AddEqualTimeIntervalsConstraints();
  auto* corner_phase = hybrid.AddModeWithInelasticImpact(
      "corner", kNumTimeSteps,
      {HybridMultibodyCollocation::ContactPair(ball, ramp1),
       HybridMultibodyCollocation::ContactPair(ball, ramp2)});
  corner_phase->AddEqualTimeIntervalsConstraints();
  ...
  // Ball is in the corner at the final state.
  const double kTol = 1e-4;
  EXPECT_NEAR(result.GetSolution(corner_phase->final_state()[0]), 0, kTol);
  EXPECT_NEAR(result.GetSolution(corner_phase->final_state()[1]), 1.15473,
              kTol);
}

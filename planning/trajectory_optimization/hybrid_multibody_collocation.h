...
/** Implements hybrid trajectory optimization using a MultibodyPlant, where the
 ...
 @code
  ...
  ContactPair heel_ground(robot_heel_geometry_id, ground_geometry_id);
  auto* heel_phase = hybrid.AddModeWithInelasticImpact(
    "heel", kNumTimeSteps, {heel_ground});
  ...
 @endcode
...
class HybridMultibodyCollocation {
 public:
  ...
  /** Implements a specialized version of Constrained Direct Collocation
   ...
   The HybridMultibodyCollocation uses one DIRCON per contact mode (and the
   contact mode is fixed inside each DIRCON). */
  class ConstrainedDirectCollocation : public MultipleShooting {
   public:
    ...
    /** Returns a mutable pointer to the Context used to evaluate the contact
    constraints at time index @p index. */
    systems::Context<AutoDiffXd>* mutable_sample_context(int index) {
      return sample_context_[index].get();
    }
   ...
   private:
    ...
    std::unique_ptr<systems::Context<AutoDiffXd>> robot_diagram_context_;
    std::vector<std::unique_ptr<systems::Context<AutoDiffXd>>>
        sample_context_;
  ...
  /** Returns the number of modes declared in the optimization. */
  int num_modes() const {
    return dircon_.size();
  }

  /** Returns the list of all possible contact pairs registered with the
  SceneGraph. Modes are defined by having a subset of these pairs active. Pairs
  that are out of contact still add constraints (to ensure that they are not in
  contact) to each mode. Using collision filter groups in the geometry engine
  can shorten this list and potentially dramatically simplify the optimization.
  */
  ...
  */
  ...
  in_contact() (e.g. bodies must only come *out* of collision). In order to add
  a mode where bodies come *into* collision, see AddModeWithInelasticImpact().
  */
  ...
  /** Returns a mutable pointer to the ith mode.
  @pre i ∈ [0, num_modes()-1]
  */
  ConstrainedDirectCollocation* mode(int i) {
    DRAKE_DEMAND(i >= 0 && i < num_modes());
    return dircon_[i].get();
  }

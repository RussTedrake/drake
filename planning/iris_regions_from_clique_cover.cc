...
VectorX<bool> MaximumClique(SparseMatrix<bool> adjacency_matrix) {
...
#pragma omp parallel for num_threads(num_threads_to_use)
  ...
  while (visibility_graph.rows() > 0) {
    VectorX<bool> clique = MaximumClique(visibility_graph);
    const int clique_size = clique.cast<int>().sum();
    ...
    int index_into_clique = 0;
    int index_into_clique_points = 0;
    for (int index_into_points : point_indices) {
      if (clique[index_into_clique++]) {
        clique_points.col(index_into_clique_points++) =
            points.col(index_into_points);
      }
    }
    ...
    if (!checker.CheckConfigCollisionFree(E.center())) {
      // TODO(russt): Find a nice solution here.  Perhaps even add the center to
      // points and call the method again (to restart from the top)?
      throw std::runtime_error(
          "The center of the circumscribed ellipsoid is in collision. This is "
          "technically possible (but should be rare); we still need to handle "
          "it better. As a workaround, providing a more dense set of points "
          "should help.");
    }
    // Add a small term to the diagonal to ensure that the ellipsoid has volume.
    ...
    // Remove the clique from point_indices.

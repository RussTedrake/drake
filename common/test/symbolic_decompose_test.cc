...
GTEST_TEST(SymbolicExtraction, DecomposeLumpedParameters) {
...

  {
    ...
    // W = [1; x], α = [a], w0 = [x; 0]
    // once we have simplification, but for now gives
    // W = [1, 0; 0, x], α = [a; a], w0 = [x; 0]
    const Vector2<Expression> f(a + x, a * x);
    const auto [W, alpha, w0] =
        DecomposeLumpedParameters(f, Vector3<Variable>{a, b, c});
    Matrix2<Expression> W_expected;
    W_expected << 1, 0, 0, x;
    EXPECT_EQ(W, W_expected);
    EXPECT_EQ(alpha, Vector2<Expression>(a, a));
...

  {
    ...
    // once we have simplification, but for now gives
    //   W = [x, x, y, y²], α = [a, b, ac], w0 = [2]
    const Expression e = a * x + b * x + a * c * y + a * c * y * y + 2;
    auto [W, alpha, w0] = DecomposeLumpedParameters(Vector1<Expression>(e),
                                                    Vector3<Variable>{a, b, c});
    EXPECT_EQ(W, RowVector4<Expression>(x, x, y, y * y));
    EXPECT_EQ(alpha, Vector4<Expression>(a, b, a * c, a * c));
...

  {
    ...
    // But it will current throw because it's not implemented yet.
